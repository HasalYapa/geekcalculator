...
export function VectorCalculator() {
  ...
  const performOperation = (op: 'add' | 'subtract' | 'dot' | 'cross' | 'angle') => {
    ...
    try {
      switch (op) {
        ...
        case 'cross':
          if (!is3D) {
              toast({ title: 'Error', description: 'Cross product is only defined for 3D vectors.', variant: 'destructive' });
              return;
          }
      ...
      const displayRes = op === 'angle' && typeof res === 'number' ? `${res.toFixed(2)}°` : res;
    ...
    <div className="space-y-8">
      ...
      <Card>
        ...
        <CardContent className="flex flex-wrap gap-2">
            ...
            <Button onClick={() => performOperation('cross')} disabled={!is3D}>Cross Product</Button>
